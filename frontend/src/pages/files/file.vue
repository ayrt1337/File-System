...
watch([simulatePreview, simulatedFormat], () => {
  ...
  const baseName = mockFile.value.name.substring(0, mockFile.value.name.lastIndexOf("."));
  mockFile.value.name = `${baseName || "4yn1T-F46gx5H61F"}.${simulatedFormat.value}`;
  
  if (simulatePreview.value) {
    if (simulatedFormat.value === "mp4") {
      mockFile.value.preview = "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4";
    } else if (simulatedFormat.value === "jpg") {
      mockFile.value.preview = "https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=1200&q=80";
    } else {
...
const handleDownload = () => {
  showToast(`Iniciando download de "${mockFile.value.name}"...`, "info");
  setTimeout(() => {
    showToast(`Download de "${mockFile.value.name}" concluído com sucesso!`, "success");
  }, 1500);
  isFileMenuOpen.value = false;
};
...
const handleToggleFavorite = () => {
  ...
  const statusText = mockFile.value.isFavorite 
...
<template>
  ...
  <div class="w-screen h-screen flex flex-col bg-[#121212] overflow-hidden text-white font-sans">
    <header class="h-[80px] bg-[#121212] border-b border-white/5 flex items-center justify-between px-6 shrink-0 relative z-30">
      ...
      <div class="flex items-center min-w-0">
        ...
        <div class="ml-4 flex flex-col">
          <div class="flex items-center gap-2 min-w-0">
            <h1 class="text-white text-base font-semibold truncate select-none" :title="mockFile.name">
              {{ mockFile.name }}
            </h1>
            <FontAwesomeIcon :icon="faFolderOpen" class="text-gray-500 text-xs shrink-0 cursor-default" />
          </div>
          ...
          <div class="flex gap-2 mt-0.5 relative">
            <div class="relative">
              ...
              >
                <span>Arquivo</span>
                <FontAwesomeIcon :icon="faChevronDown" class="text-[9px] transition-transform duration-200" :class="{'rotate-180': isFileMenuOpen}" />
              </button>
              ...
              <Transition name="dropdown-fade">
                ...
                >
                  ...
                  <button 
                    ...
                    :class="mockFile.isFavorite ? 'text-[#fbbf24] hover:bg-[#fbbf24]/5' : 'text-gray-300 hover:text-white hover:bg-white/5'"
                  >
                    <FontAwesomeIcon :icon="mockFile.isFavorite ? faStar : faStarRegular" class="w-4 h-4" />
                    <span>{{ mockFile.isFavorite ? 'Remover dos favoritos' : 'Adicionar aos favoritos' }}</span>
                  </button>
                </div>
              </Transition>
            </div>
      ...
      <div class="flex items-center shrink-0">
        <button 
          @click="handleShareClick"
          class="flex items-center gap-2 bg-[#1a73e8] hover:bg-[#155cb0] text-white px-5 py-2 rounded-full font-semibold text-sm transition-all duration-300 shadow-md cursor-pointer mr-5 active:scale-95"
        >
          <FontAwesomeIcon :icon="faLock" class="text-xs" />
          <span>Compartilhar</span>
          <FontAwesomeIcon :icon="faChevronDown" class="text-[9px] opacity-80" />
        </button>
    ...
    <main class="flex-1 bg-[#161616] m-4 mt-2 rounded-[24px] border border-white/5 flex flex-col items-center justify-center relative overflow-hidden p-6 z-10">
      <div v-if="mockFile.preview" class="w-full h-full flex flex-col items-center justify-center">
        <div v-if="mockFile.format === 'mp4'" class="max-w-[1000px] w-full max-h-[75vh] flex items-center justify-center bg-black rounded-2xl overflow-hidden shadow-2xl relative border border-white/10">
          <video 
            :src="mockFile.preview" 
            controls 
            autoplay 
        ...
        <div v-else-if="['jpg', 'jpeg', 'png', 'webp', 'gif'].includes(mockFile.format)" class="max-w-[900px] w-full max-h-[75vh] flex items-center justify-center rounded-2xl overflow-hidden shadow-2xl bg-black/40 p-2 border border-white/10">
          <img 
            :src="mockFile.preview" 
      ...
      <div v-else class="flex flex-col items-center justify-center text-center gap-6 animate-fade-in max-w-lg px-6">
        <div 
        ...
        <div class="flex flex-col gap-2">
          <h2 class="text-white text-xl font-bold truncate max-w-sm select-all">{{ mockFile.name }}</h2>
          <p class="text-gray-400 text-sm">
            Nenhuma visualização disponível para arquivos do tipo <span class="uppercase font-semibold text-white/80">.{{ mockFile.format }}</span>
          </p>
        </div>
      ...
      <div class="absolute bottom-4 left-4 bg-black/85 backdrop-blur-md border border-white/10 rounded-2xl p-4 flex flex-col gap-3 shadow-2xl text-xs z-30 transition-all hover:border-white/20">
        <div class="flex items-center justify-between gap-6 border-b border-white/10 pb-1.5">
          <span class="font-bold text-white tracking-wide uppercase text-[10px]">Configurações do Mock</span>
        ...
        <div class="flex items-center justify-between gap-4">
          ...
          <button 
            ...
            :class="simulatePreview ? 'bg-[#009900]/20 text-[#22c55e] border border-[#009900]/30' : 'bg-white/5 text-gray-400 border border-white/10'"
          >
            <FontAwesomeIcon :icon="simulatePreview ? faEye : faEyeSlash" />
            <span>{{ simulatePreview ? "Simulando Com" : "Simulando Sem" }}</span>
          </button>
        </div>
