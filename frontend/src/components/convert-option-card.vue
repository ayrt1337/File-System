...
const fromIcon = computed(() => {
  ...
  if (props.type === "image" || ext === "jpg" || ext === "png") return faFileImage;
...
const colorClasses = computed(() => {
  switch (props.type) {
    ...
    default:
      return {
        fromColor: props.from.toLowerCase() === "docx" ? "text-blue-600" : "text-green-500",
        toColor: props.to.toLowerCase() === "pdf" ? "text-red-500" : "text-green-600",
  ...
  >
    <div class="flex items-center gap-4">
      <div class="relative w-12 h-10 flex items-center justify-center">
        <div :class="['absolute top-0 left-0 text-xl opacity-75', colorClasses.fromColor]">
          <FontAwesomeIcon :icon="fromIcon" />
        </div>
        
        <div class="absolute inset-0 flex items-center justify-center text-[10px] text-gray-600 z-10 rotate-[25deg]">
          <FontAwesomeIcon :icon="faArrowRight" />
        </div>
        
        <div :class="['absolute bottom-0 right-0 text-xl', colorClasses.toColor]">
          <FontAwesomeIcon :icon="toIcon" />
        </div>
      </div>

      <div class="flex flex-col">
        <span class="text-white font-semibold text-sm group-hover:text-white transition-colors duration-200 uppercase tracking-wide">
          {{ from }} <span class="text-gray-500 lowercase font-normal px-0.5">para</span> {{ to }}
        </span>
        <span class="text-gray-500 text-[11px] font-normal group-hover:text-gray-400 transition-colors duration-200">
          {{ title }}
        </span>
      </div>
    </div>
